...
- [eight-eyes](https://github.com/AgentBuildersApp/eight-eyes) by [AgentBuildersApp](https://github.com/AgentBuildersApp) - Hook-enforced multi-agent code review for Claude Code, Copilot CLI, and Codex CLI. Splits a code change into 8 constrained roles — the skeptic reviews blind without the implementer’s narrative, the security auditor cannot edit files, the verifier must prove acceptance criteria with evidence. Hooks enforce scope boundaries, not just prompts. 142 tests, stdlib-only Python, zero dependencies.
